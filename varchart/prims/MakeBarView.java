...
public class MakeBarView extends DefaultReporter {
	...
	public Object report(Argument[] args, final Context context)
		...
		if ( title.length() == 0 ) { title = "Patch Reporter Results (Bar)"; }
		...
		final BarView manualBarView = new BarView(title, task, id);
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					...
					manualBarView.setupForRendering( worldWidth, worldHeight );
					manualBarView.manuallyRefreshReporterView(context);
					manualBarView.setVisible(true);
				}
			});
		...
		int i = VarchartExtension.storeWindowAtIndex(id, manualBarView);
		if (i < 0 ) {
